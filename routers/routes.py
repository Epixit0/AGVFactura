...
async def enforce_login(request: Request):
    ...
    # IMPORTANTE: Restaurar la función original antes de pasar a producción
    # Para restaurar, elimina este bloque y descomenta el código original a continuación
    ...
    # CÓDIGO ORIGINAL (comentado durante desarrollo):
    """
...
# Ruta raíz - Redirecciona a la página de inicio
@router.get("/", response_class=HTMLResponse)
async def root(request: Request):
    log_request_info(request, "Accediendo a la página de inicio")
    user_info, redirect = await enforce_login(request)
    if redirect:
        return redirect
    
    app_logger.info(
        "Acceso permitido a la página de inicio",
        extra={
            "device": request.headers.get("User-Agent", "UnknownDevice"),
...
async def contacto(request: Request):
    log_request_info(request, "Accediendo a la página de contacto")
    user_info, redirect = await enforce_login(request)
    if redirect:
        return redirect
    
    app_logger.info(
        "Acceso permitido a la página de contacto",
        extra={
            "device": request.headers.get("User-Agent", "UnknownDevice"),
            "user": user_info.get("preferred_username", "UnknownUser"),
            "ip": request.client.host,
...
# Añadir esta ruta después de la ruta de contacto y antes de las rutas de creación de documentos

@router.get("/login", response_class=HTMLResponse)
async def login_page(request: Request):
    log_request_info(request, "Accediendo a la página de login")
    
    # No requerimos autenticación para la página de login
...
async def dev_dashboard(request: Request):
    """Ruta de desarrollo que permite acceder al dashboard sin autenticación"""
    log_request_info(request, "Accediendo al dashboard en modo desarrollo")
    
    # Crear un diccionario con datos de usuario de prueba
    mock_user_info = {
        "email": "dev@example.com",
        "preferred_username": "Developer",
        "groups": ["admin"],
    ...
    template = env.get_template("dashboard.html")
    return template.render(user_info=mock_user_info)

@router.get("/administration", response_class=HTMLResponse)
async def administration_panel(request: Request):
    log_request_info(request, "Accediendo al panel de administración")
    user_info, redirect = await enforce_login(request)
    if redirect:
        return redirect
